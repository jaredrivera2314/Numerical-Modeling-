function [ X Y ] = Heun( X0, X_end, Y0, h )
%==========================================================================
% CEE103 Heun's Method
% Chen, Peng-Yu
% Originial Coding
% 05/30/2017
%==========================================================================
% This function utilized Heun's method to solve ODE problem.Users need to 
...
Y(1)=Y0;
for i=2:length(X)
    Y(i)=Y(i-1)+(h/2)*(fcn(X(i-1),Y(i-1))+fcn(X(i),(Y(i-1)+h*fcn(X(i-1),Y(i-1))))); % function need to be modified
end
exact=(2*X+1)./(X.^2+1);                     % Has been derived through analytical analysis
Error=exact-Y;                               % Error=Exact_Value- App_Value
display('x       y_approximate     y_exact      Heun_Error')
for i=1:n
