function [ X Y ] = Trap_N( X0, X_end, Y0, h, tol )
%==========================================================================
% CEE103 Trapzoidal Method_Newton
% Chen, Peng-Yu
% Originial Coding
% 05/31/2017
%==========================================================================
...
% Start iteration
while i<=n;
    Yt1=Y(i-1)+h*(fcn(X(i-1),Y(i-1)));              % use forward to guess Yn+1
    it_count=0;                                     % count for iteration
    error=1;                                        % initial error
    while abs(error)>tol 
        Fx=Yt1-Y(i-1)-(h/2)*(fcn(X(i-1),Y(i-1))+fcn(X(i),Yt1)); % Form the function in Newton'smethod
        dFx=1+(h/2)*(2*X(i)/(X(i)^2+1));                                                  % first deritive
...
display('x       y_approximate     y_exact      Trap_N_Error')
for i=1:n
